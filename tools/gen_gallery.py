...
def frame(inner: str, title: str, accent: str) -> str:
    return (
...
def ranked_bars(items, ramp, unit='', signed=False):
...
def grouped_bars(categories, label_a, label_b, ramp):
...
def line_band(series, band_lo, band_hi, anom_start, anom_label, ramp):
...
def calibration(points, precision, ramp):
...
def matrix(labels, abstain, accent, ramp):
    n = len(labels)
...
def process_before_after(before_n, after_n, note, ramp):
    x0, y0 = 140, 640
...
def wireframe(tiles, table_label, ramp):
...
def donut_and_line(donut, line_series, line_unit, ramp):
    cx, cy, r = 320, 380, 220
...
SPECS = {
    ...
    'ocean-drones': [
        ('telemetry', lambda accent, ramp: frame(line_band(
            ...
            7.95, 8.05, 8, 'divergence flagged', ramp), 'pH vs seasonal baseline', accent)),
        ('review queue', lambda accent, ramp: frame(queue_list([
        ...
        ], ramp, unit='%'), 'Classification review queue', accent)),
    ],
    'retail-demand-forecast': [
        ('category accuracy', lambda accent, ramp: frame(grouped_bars([
            ('Bakery', 91, 76), ('Chilled meats', 84, 79), ('Produce', 79, 71),
            ('Dairy', 88, 82), ('Frozen', 86, 80),
        ], 'Model', 'Seasonal-naive', ramp), 'Accuracy by category (%)', accent)),
        ('weekly review', lambda accent, ramp: frame(waste_availability(
            [('W1', 6.4), ('W4', 5.6), ('W8', 4.7), ('W12', 4.1)],
            [('W1', 93.1), ('W4', 95.0), ('W8', 96.4), ('W12', 97.6)],
        ), 'Waste and availability, weekly', accent)),
    ],
    'churn-early-warning': [
        ('calibration', lambda accent, ramp: frame(calibration([
            (0.05, 0.06), (0.15, 0.14), (0.25, 0.27), (0.35, 0.33), (0.45, 0.46),
            (0.55, 0.53), (0.65, 0.67), (0.75, 0.74), (0.85, 0.86), (0.95, 0.93),
        ], 0.46, ramp), 'Calibration curve', accent)),
        ('drivers', lambda accent, ramp: frame(ranked_bars([
        ...
        ], ramp, signed=True), 'Per-account risk drivers', accent)),
    ],
    'finance-close-automation': [
        ('exceptions', lambda accent, ramp: frame(table(
            ...
            ], [380, 260, 460]), 'Reconciliation exception queue', accent)),
        ('before after', lambda accent, ramp: frame(process_before_after(61, 9, 'Eleven duplicated steps and four unread outputs deleted first', ramp), 'Steps before and after', accent)),
    ],
    'logistics-control-tower': [
        ('lane performance', lambda accent, ramp: frame(ranked_bars([
        ...
        ], ramp, unit='%'), 'Exception rate by lane', accent)),
        ('floor display', lambda accent, ramp: frame(big_tiles([
            (14, 'Open exceptions right now'),
            ('Rotterdam→Cork', 'Worst lane this week'),
            ('6h 40m', 'Oldest open consignment'),
        ]), 'Floor display', accent)),
    ],
    'clinic-nlp-triage': [
        ('confusion matrix', lambda accent, ramp: frame(matrix(
            ['Appt change', 'Prescription', 'Billing', 'General', 'Clinical', 'Complaint'],
            [0.03, 0.05, 0.04, 0.11, 0.02, 0.07], accent, ramp,
        ), 'Confusion matrix · six of nine intents', accent)),
        ('audit log', lambda accent, ramp: frame(table(
            ...
            ], [560, 220, 320]), 'Routing audit log', accent)),
    ],
    'energy-anomaly-detection': [
        ('meter baseline', lambda accent, ramp: frame(line_band(
            ...
            38, 43, 6, 'chiller running unoccupied', ramp), 'Half-hourly load vs baseline', accent)),
        ('alert queue', lambda accent, ramp: frame(queue_list([
        ...
        ], ramp, unit='/day'), 'Alert queue, ranked by €/day', accent)),
    ],
}
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for slug, slots in SPECS.items():
            ...
            ramp = hex_ramp(accent, accent2)
            for i, (_, builder) in enumerate(slots, start=1):
                svg_content = builder(accent, ramp)
